...
document.addEventListener("DOMContentLoaded", () => {
  ...
  gameCards.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      if (gameName === "cannon-ball") {
        window.location.href = "games/cannon-ball/cannon.html";
      } else {
...
function loadHighScores() {
  const games = [
  'snake','tictactoe','flappy','pong','memory','breakout','rps',
  'minesweeper','whackamole','2048','racing','math','wordle',
  'picsword','sliding','bubble','dart','colorswitch',
  'cannon-ball'
];
  ...
  games.forEach((game) => {
    ...
    if (element) {
      if (score) {
        ...
        element.textContent =
          game === "memory" || game === "minesweeper" ? "--" : "0";
      }
    }
  });
}
